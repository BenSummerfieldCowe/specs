...
//TODO double check, but it seems that if change so that we get the repo url rather than the folder name, then every time
  ...
  console.log('in name thing ' + repoName);
  console.log('in name thing ' + projectData.repoUrl.replace(/https:\/\//,'').replace(/\//g,'.'));
  console.log('did i change it? '+  projectData.repoUrl);
  // }
