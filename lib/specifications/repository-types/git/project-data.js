...
function getProjectData(projectData, targetBranchName) {
  ...
  var headCommit;
  var listOfFiles;
  //TODO check this regex, think it might be able to be better
  projectData.repoNameUnHashed = projectData.repoName;
  var repoProjName = /^.+\.([^\.]+)\.([^\.]+)/.exec(projectData.repoName);
  projectData.repoProjName = repoProjName[1]+'/'+repoProjName[2];
  console.log('ok so we come in here and checking the new thing' + projectData.repoProjName);
...
function generateRepoMetaData(projectData, sha, tagsAndBranches, currentBranchName, files, remoteUrl) {
  var updatedData = {
    ...
    repoNameUnHashed: projectData.repoNameUnHashed,
    repoProjName: projectData.repoProjName,
    // New
    commit: sha,
